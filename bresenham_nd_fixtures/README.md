...
Optional: pass `--rust-bin /path/to/ldcheck` to assert the 3-D Zingl field
matches a `line_drawing` Bresenham3d CLI. The fixtures themselves store the
Python Zingl / raster_geometry sequences, which match that crate on the corpus.
...
| **line_drawing** (Rust) `Bresenham3d` | 3-D only | Cargo crate 1.0.1. No generic N-D API. Same rule as Zingl / raster_geometry. |
